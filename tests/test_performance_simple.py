...
class TestPerformance:
    ...
    def test_write_cache_db_comparison(self, sync_cache, json_generator):
        ...
        print(
            f"2. CACHE HIT (warm cache):            {cache_warm_time * 1000:8.2f}ms "
            f"({write_time / cache_warm_time:5.1f}x faster)"
        )
        print(
            f"3. DB READ (cache miss, DB hit):      {db_read_time * 1000:8.2f}ms "
            f"({write_time / db_read_time:5.1f}x vs write)"
        )
